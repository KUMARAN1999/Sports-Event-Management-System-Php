<!doctype html>
    ...
    <body>
      ...
      $result = mysqli_query($connector,"SELECT * FROM register ");
      ?>
      ...
      <tbody>
        <?php
          while( $row = mysqli_fetch_assoc( $result ) ){
            echo
            "<tr><td>{$row['registration']}</td><td>{$row['name']}</td><td>{$row['dept']}</td><td>{$row['year']}</td><td>{$row['sport']}</td><td>{$row['phone']}</td><td>{$row['mail']}</td></tr>\n";
          }
        ?>
      </tbody>
    </table>
